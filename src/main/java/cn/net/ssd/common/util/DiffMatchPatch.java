...
public class DiffMatchPatch {
    ...
    /**
     * 主函数示例，演示了如何使用DiffMatchPatch库进行文本差异比较和格式化。
     *
    ...
    public static void main(String args[]) {
        ...
        // 执行文本比较
        LinkedList<DiffMatchPatch.Diff> diff = dmp.diff_main("有的同学对语言很兴趣。每个青年都应当有远大的报复", "有的同学对语言很有兴趣。每个青年都应当有远大的抱负");
        ...
        // 根据差异结果创建带标记的文本（插入、删除、相等操作的标记）
        StringBuilder annotatedText = new StringBuilder();
        for (DiffMatchPatch.Diff aDiff : diff) {
            // 根据操作类型，为文本添加相应的HTML标签以标示差异
            switch (aDiff.operation) {
                case INSERT:
                    annotatedText.append("<ins>").append(aDiff.text).append("</ins>");
                    break;
                case DELETE:
                    annotatedText.append("<del>").append(aDiff.text).append("</del>");
                    break;
                case EQUAL:
                    annotatedText.append("<span>").append(aDiff.text).append("</span>");
            }
        }
        // 打印带标记的文本
        System.out.println(annotatedText.toString());
    }
