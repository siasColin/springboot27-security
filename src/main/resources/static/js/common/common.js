    ...
    ajax: function (url, paras, sy, tp, next) {
        ...
        $.ajax({
            ...
            success: function (rsp) {
                ...
                } else if (rsp.code == Common.sessionInvalidCode) {//会话超时
                    Common.openConfirm(rsp.msg, function () {
                        window.location.replace(Common.ctxPath + Common.sessionInvalidURL);
                    });
                } else if (rsp.code == '006') {//当前登录用户关联信息已更新，提示用户重新登录
                    layui.use(['layer'], function () {
                        var layer = layui.layer
                        var logoutTipIndex = layer.confirm(rsp.msg, {
                            btn: ['确定'], //按钮
                            shade: 0.5
                        }, function () {
            ...
            error: function (rsp) {
                console.log("closeLoad error:" + url);
                setTimeout(function () {
                    window.parent.Common.closeload();
                }, 200);
